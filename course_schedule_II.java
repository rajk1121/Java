...
public class course_schedule_II {
    ...
    public static int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for(int i=0 ; i<numCourses; i++) {
        	if(!visited[i]) {
        		prepareChild(set2, set, i, visited);
            	visited[i] = true;
        	}
        }
    ...
    private static void prepareChild(HashMap<Integer, Integer> set2, HashMap<Integer, List<Integer>> set, int i, boolean[] visited) {
    	...
    	for(int j=0 ; j<set.get(i).size() ; j++) {
    		prepareChild(set2, set, set.get(i).get(j), visited);
...
;
    	 int[] a = findOrder(800, prerequisites);
